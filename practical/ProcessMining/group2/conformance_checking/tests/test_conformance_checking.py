...
def test_pipeline():
    ...
    for trace_id, activities in trace_data.items():
        ...
        for activity in activities:
            event = Event({'concept:name': activity})
            trace.append(event)
        input_log.append(trace)
    comparison = Comparison()
    input_fpm, replayed_fpm = comparison.pipeline(
        input_log, AlgoPm4Py.ALPHA, variants.basic_playout
    )

    assert input_fpm.relations == {
        'a': {'a': '#', 'b': '->', 'c': '#', 'd': '||'},
        'b': {'a': '<-', 'b': '#', 'c': '->', 'd': '#'},
        'c': {'a': '#', 'b': '<-', 'c': '#', 'd': '||'},
        'd': {'a': '||', 'b': '#', 'c': '||', 'd': '#'},
    }
    assert replayed_fpm.relations == {
        'a': {'a': '#', 'd': '#'},
        'd': {'a': '#', 'd': '#'},
    }
