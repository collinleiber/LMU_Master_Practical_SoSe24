...
@pytest.fixture
def footprints(event_log):
    footprints = pm4py.discovery.discover_footprints(event_log)
    # convert to set (discard frequency information of dictionary as our implementation does provide that)
    footprints['dfg'] = set(footprints['dfg'].keys())
    return footprints


def test_footprints_discovery(alpha_miner, footprints):
